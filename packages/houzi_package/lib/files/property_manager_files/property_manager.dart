...
import 'package:houzi/services/postgresql_property_service.dart';
...
class PropertyManager with ChangeNotifier {
  ...
  uploadProperty()  async {
    ...
    if (_propertyDataMap.isNotEmpty){
      ...
      ApiResponse? nonceResponse = await _apiManager.fetchAddImageNonceResponse();
        ...
        if(totalFloorPlansPendingImages > 0){
          ...
          for (int i = 0; i < floorPlansList.length; i++) {
            ...
            if (imageStatus != null && imageStatus == PENDING) {
              ...
              ApiRequest request = _apiManager.uploadListingImagesApi(
                userToken: HiveStorageManager.getUserToken() ?? "",
              ...
              if (imagePath != null) {
                ...
                //     url: '${_apiManager.provideSavePropertyImagesApi()}',
                //     files: [
                //       FileItem(
                //         path: imagePath,
  ...
  Future<void> uploadPropertyViaDio(Map<String, dynamic> _propertyDataMap) async {
    ...
    if (nonce == null || nonce.isEmpty) {
      ...
      if (isPropertyForUpdate) {
        nonceResponse = await _apiManager.fetchUpdatePropertyNonceResponse();
      } else {
        nonceResponse = await _apiManager.fetchAddPropertyNonceResponse();
      }
    ...
    // ==================== POSTGRESQL INTEGRATION ====================
    // Upload to PostgreSQL database FIRST (our custom integration)
    print('🚀 Uploading to PostgreSQL database...');
    Map<String, dynamic> postgresResult = await PostgreSQLPropertyService.uploadProperty(_propertyDataMap);
    
    bool postgresSuccess = postgresResult['success'] == true;
    if (postgresSuccess) {
      print('✅ PostgreSQL upload successful!');
      print('   Property ID: ${postgresResult['property_id']}');
    } else {
      print('❌ PostgreSQL upload failed: ${postgresResult['message']}');
    }
    // ================================================================

    // ==================== WORDPRESS INTEGRATION (COMMENTED OUT) ====================
    // NOTE: The original WordPress integration code is kept below but will be skipped.
    // We're now using PostgreSQL instead. To re-enable WordPress, uncomment the code
    // below and comment out the PostgreSQL section above.
    
    // Response response;
    ApiResponse<AddOrUpdateListing?> response;
    
    // Skip WordPress upload if PostgreSQL was successful
    if (!postgresSuccess) {
      // Only try WordPress as fallback if PostgreSQL failed
      if (isPropertyForUpdate){
        if (kDebugMode && enableUploadPropertyLogs) {
          print('Updating Property via WordPress (fallback)...');
        }
        response = await _apiManager.updateListing(_propertyDataMap, nonce!);
      }
      else {
        if (kDebugMode && enableUploadPropertyLogs) {
          print('Uploading Property via WordPress (fallback)...');
        }
        response = await _apiManager.addNewListing(_propertyDataMap, nonce!);
      }
    } else {
      // PostgreSQL succeeded, fake a WordPress response to continue flow
      if (kDebugMode && enableUploadPropertyLogs) {
        print('Skipping WordPress upload (using PostgreSQL instead)');
      }
      response = ApiResponse<AddOrUpdateListing?>(
        success: true,
        message: 'Property uploaded to PostgreSQL',
        internet: true,
        result: AddOrUpdateListing(
          propId: int.tryParse(postgresResult['property_id']?.toString() ?? '0') ?? 0,
        ),
      );
    }
    // ===============================================================================
    ...
    if (kDebugMode && enableUploadPropertyLogs) {
      ...
      print("Response Internet: ${response.internet}");
      print("Response Property Id: ${response.result != null ? response.result!.propId : ""}");
    }

    if (response.success && response.internet && response.result != null) {

      AddOrUpdateListing addOrUpdateListing = response.result!;
      ...
      _uploadedPropertyId = addOrUpdateListing.propId;
      _totalProgress = 100;
      ...
      if (kDebugMode && enableUploadPropertyLogs) {
        print('PropertyManager: got a dio error ${response.message}');
      }
    }
  }
...
// import 'package:houzi_package/blocs/api_manager.dart';
// import 'package:houzi_package/common/constants.dart';
// import 'package:houzi_package/files/generic_methods/utility_methods.dart';
// import 'package:houzi_package/files/hive_storage_files/hive_storage_manager.dart';
//
...
//       response = await _apiManager.fetchUpdatePropertyResponse(_propertyDataMap);
//     }
//     else {
//       // print('Uploading Property...');
//       response = await _apiManager.fetchAddPropertyResponse(_propertyDataMap);
//     }
//
