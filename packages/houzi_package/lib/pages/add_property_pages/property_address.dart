...
import 'package:houzi_package/api_management/parsers/places_api_parser.dart';
...
import 'package:houzi_package/widgets/caricom_country_dropdown.dart';
...
class PropertyAddressPageState extends State<PropertyAddressPage>{
  ...
  Map<String, dynamic>? _selectedCountryData;
                ...
                children: [
                  LocationFromMapWidget(
                    ...
                    listener: ({currentPosition, dataInfoMap}) {
                      ...
                      if (dataInfoMap != null) {
                        if (dataInfoMap.containsKey(ADD_PROPERTY_MAP_ADDRESS)) {
                          _addressTextController.text = dataInfoMap[ADD_PROPERTY_MAP_ADDRESS] ?? "";
                        }
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Center(
            child: Text(
              'COUNTRY DROPDOWN SHOULD BE HERE',
  ...
  updateFormFields(LatLng? location) async {
    if (location != null) {
      ...
      if (tempMap != null && tempMap.isNotEmpty &&
        ...
        addressDataList = PlacesApiParser.parseAddressFromJson(jsonEncode(tempMap["results"]));
...
class _LocationFromMapWidgetState extends State<LocationFromMapWidget> {
  ...
  LatLng? currentPosition;
  LatLng defaultLocation = const LatLng(16.0, -62.0); // Caribbean center coordinates
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          CaricomCountryDropdown(
            onCountrySelected: (countryData) {
              if (mounted) {
                setState(() {
                  dataMap[ADD_PROPERTY_COUNTRY] = countryData['name'];
                  dataMap['country'] = countryData['name'];
                  dataMap['country_id'] = countryData['id'];
                  dataMap['country_code'] = countryData['code'];
                });
      ...
      child: TextField(
        ...
        onTap: () async {
          ...
          if (result != null && result.placeId != null) {
            ApiResponse<PlaceDetails?> response =
            await PlaceApiProvider.getPlaceDetailFromPlaceId(result.placeId!);
            if (response.success && response.result != null) {
              if (mounted) {
                PlaceDetails placeDetails = response.result!;
                String? mapAddress = placeDetails.location;

                  double? latitude = placeDetails.latitude;
                  double? longitude = placeDetails.longitude;
                  ...
                  if (latitude != null && longitude != null) {
                    ...
                    if (currentPosition != null) {
                      ...
                      dataMap[ADD_PROPERTY_LATITUDE]  = "$latitude";
                      dataMap[ADD_PROPERTY_LONGITUDE]  = "$longitude";
                      dataMap[ADD_PROPERTY_MAP_ADDRESS]  = mapAddress;
                      dataMap[ADD_PROPERTY_MAP_ADDRESS+"temp"]  = mapAddress;

                      setState(() {
                        _searchResultController.text = placeDetails.location ?? "";
                        widget.listener(dataInfoMap: dataMap);
                      });
                    }
                  }
...
class _AddressFromMapState extends State<AddressFromMap> {
  ...
  @override
  void initState() {
    _initialPosition = widget.initialLatLng ?? const LatLng(16.0, -62.0); // Fallback to Caribbean center
