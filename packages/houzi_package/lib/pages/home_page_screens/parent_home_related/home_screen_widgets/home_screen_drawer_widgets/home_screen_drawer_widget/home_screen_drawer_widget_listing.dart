...
class _HomeScreenDrawerWidgetsListingState extends State<HomeScreenDrawerWidgetsListing> {
  ...
  void initState() {
    ...
    if (widget.drawerItemMap is DrawerItem) {
      ...
      drawerConfigItemMap = _apiManager.convertDrawerLayoutItemToJson(widget.drawerItemMap);
    } else {
            ...
            children: [
              if (drawerConfigItemMap[sectionTypeKey] == PLACE_HOLDER_SECTION_TYPE)
                ...
                getListTile(
                  ...
                  widget.userInfoMap[USER_ROLE] != USER_ROLE_BUYER_VALUE)
                getExpansionTile(
                  drawerConfigItemMap[titleKey] ?? "",
  ...
  Widget getListTile({
    ...
    // Filter out unwanted home options: Home Elegant (home_1), Home Location (home_2), Home Tabbed (home_3)
    // Only keep Home Carousel (home_0)
    if (itemSectionType == home01SectionType || 
        itemSectionType == home02SectionType || 
        itemSectionType == home03SectionType) {
      return Container(); // Return empty container to hide these menu items
    }

    // Filter out Blogs menu item
    if (itemSectionType == blogsKey) {
      return Container(); // Hide blogs from menu
    }

    // Filter out Agencies and Agents (WordPress/internet-dependent)
    // We're running locally with PostgreSQL, these require WordPress API
    if (itemSectionType == agenciesSectionType || itemSectionType == agentsSectionType) {
      return Container(); // Hide WordPress-dependent items
    }

    // ROLE-BASED FILTERING: Hide Add Property options for guests
    String userRole = userInfoMap[USER_ROLE] ?? "";
    if (itemSectionType == addPropertySectionType || itemSectionType == quickAddPropertySectionType) {
      // Only show Add Property options to agencies, not guests
      if (userRole == USER_ROLE_BUYER_VALUE || userRole == 'guest') {
        print('🚫 Hiding Add Property for guest user');
        return Container(); // Hide add property options for guests
      }
    ...
    if (itemSectionType == logoutSectionType && isUserLogged) {
      ...
      if (itemSectionType == loginSectionType && isUserLogged) {
        ...
        if (userInfoMap[USER_ROLE] == USER_ROLE_BUYER_VALUE) {
        ...
        else if(itemSectionType == myAgentsSectionType && userInfoMap[USER_ROLE] != USER_ROLE_AGENCY_VALUE){
          return Container();
