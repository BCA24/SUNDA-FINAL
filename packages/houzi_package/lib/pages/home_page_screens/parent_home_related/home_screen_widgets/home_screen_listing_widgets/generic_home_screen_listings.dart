...
import 'package:houzi_package/widgets/caricom_region_explorer.dart';
import 'package:houzi_package/widgets/test_house_carousel.dart';
...
class _HomeScreenListingsWidgetState extends State<HomeScreenListingsWidget>  with AutomaticKeepAliveClientMixin<HomeScreenListingsWidget>  {
  ...
  void initState() {
    ...
    generalNotifierLister = () {
      ...
      else if (GeneralNotifier().change == GeneralNotifier.CUSTOM_SEARCH_ON_HOME) {
    ...
    try {
      ...
      /// Fetch Properties
        ...
        if (UtilityMethods.listDependsOnUserSelection(homeConfigMap)) {
          ...
          Map<String, dynamic> searchMap = HiveStorageManager.readHomeCustomSearchInfo();
          if (isCustomSearched(searchMap)) {
            dataMap.addAll(searchMap);
            setRouteRelatedDataMap.addAll(searchMap);
          }
        }
      ...
      /// Fetch CARICOM Explorer
      else if (homeConfigMap[sectionTypeKey] == "caricom_explorer") {
        tempList = [1]; // Static data, no API call needed
      }
  ...
  Widget build(BuildContext context) {
    print("🔍 GenericHomeScreenListings: section_type = ${homeConfigMap[sectionTypeKey]}");
    super.build(context);
          ...
          return noDataReceived
              ...
              : Column(
                  children: [
                        ...
                        children: [
                          Expanded(
                            ...
                            // child: Home02HeaderWidget(
                              ...
                              padding: const EdgeInsets.fromLTRB(20.0, 0.0, 20.0, 0.0), // Zero padding for maximum height
                            ),
                          ),

                          // Removed "See All >" button as requested
                        ],
                      ),
                    ...
                    if (homeConfigMap[sectionTypeKey] == "caricom_explorer")
                      const CaricomRegionExplorer(),

                    if (homeConfigMap[sectionTypeKey] == "test_houses")
                      const TestHouseCarousel(),
                  ],
          );
        });
      }
  ...
  bool isCustomSearched(Map<String, dynamic> map) {
    if (map.isNotEmpty &&
