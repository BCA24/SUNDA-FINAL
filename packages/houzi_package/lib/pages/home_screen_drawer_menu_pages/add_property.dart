...
class AddPropertyState extends State<AddProperty> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    if (!widget.isPropertyForUpdate && !widget.isDraftProperty &&
        _userRole == USER_ROLE_AGENT_VALUE) {
    ...
    _addPropertyPages = [
      PropertyInformationPage(
        ...
        propertyInformationPageListener: (Map<String, dynamic> map){
          print('🎯🎯🎯 PropertyInformationPageListener CALLED! Received ${map.length} keys');
          print('🔑 Keys received: ${map.keys.toList()}');
          print('💰 Price keys check: sale_price=${map['sale_price']}, ADD_PROPERTY_PRICE=${map[ADD_PROPERTY_PRICE]}');
          if (mounted) {
            setState(() {
              tempPropertyInformationPageMap = map;
              addPropertyDataMap.addAll(tempPropertyInformationPageMap);
              print('✅ addPropertyDataMap now has ${addPropertyDataMap.length} keys after merge');
              print('💰 After merge: sale_price=${addPropertyDataMap['sale_price']}');
            });
          }
        },
      ),
    ...
    if (_userRole == ROLE_ADMINISTRATOR ||
        _userRole == USER_ROLE_AGENCY_VALUE) {
      if(mounted) setState(() {
  ...
  fetchNonce() async {
    ...
    if (widget.isPropertyForUpdate) {
      response = await _apiManager.fetchUpdatePropertyNonceResponse();
    } else {
      response = await _apiManager.fetchAddPropertyNonceResponse();
    }
                        ...
                        child: ButtonWidget(
                          ...
                          onPressed: () async {
                            ...
                            if(_currentPageType == PropertyInformationPage){
                              print('📝 Validating PropertyInformationPage form...');
                              if(formKey01.currentState!.validate()){
                                print('✅ PropertyInformationPage validation PASSED');
                                print('💾 Calling formKey01.currentState!.save()...');
                                // CRITICAL FIX: Call save() to trigger onSaved callbacks!
                                formKey01.currentState!.save();
                                
                                // EMERGENCY WORKAROUND: Manually check if price data is missing and add it
                                print('🔍 Checking dataMap for price keys after save:');
                                print('   sale_price: ${addPropertyDataMap['sale_price']}');
                                print('   ADD_PROPERTY_PRICE: ${addPropertyDataMap[ADD_PROPERTY_PRICE]}');
                                
                                // If price is still missing, try to get it from tempPropertyInformationPageMap
                                if (!addPropertyDataMap.containsKey('sale_price') || addPropertyDataMap['sale_price'] == null) {
                                  print('⚠️ Price missing! Attempting emergency fix...');
                                  print('🔍 tempPropertyInformationPageMap keys: ${tempPropertyInformationPageMap.keys.toList()}');
                                  print('🔍 tempPropertyInformationPageMap: $tempPropertyInformationPageMap');
                                }
                                print('✅ save() completed');
                              }else{
                                print('❌ PropertyInformationPage validation FAILED');
                                _isPageValidated = false;
                              }
  ...
  startAddPropertyProcess(){
    ...
    // 🔥 DEBUG: Print final data map before upload
    print('🔥🔥🔥 FINAL addPropertyDataMap in AddProperty controller:');
    print('🔥 Keys in map: ${addPropertyDataMap.keys.toList()}');
    print('🔥 Checking price keys:');
    print('   ADD_PROPERTY_PRICE: ${addPropertyDataMap[ADD_PROPERTY_PRICE]}');
    print('   sale_price: ${addPropertyDataMap['sale_price']}');
    print('   fave_property_price: ${addPropertyDataMap['fave_property_price']}');
    print('   prop_price: ${addPropertyDataMap['prop_price']}');
    print('   price: ${addPropertyDataMap['price']}');
    ...
    if(isLoggedIn == true){
      ...
      // 🔥 CLEAR OLD QUEUED PROPERTIES FIRST!
      print('🧹 Clearing old queued properties...');
      _addPropertiesDataMapsList.clear();
      print('✅ Queue cleared!');
      ...
      print('💾 Stored NEW property to queue with price data');
      /// Upload Property:
