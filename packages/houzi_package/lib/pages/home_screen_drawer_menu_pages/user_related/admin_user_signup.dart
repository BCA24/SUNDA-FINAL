...
class AdminUserSignUpState extends State<AdminUserSignUp> with ValidationMixin {
  ...
  void initState() {
    ...
    // Set default role if only one role exists or set a default if none exist
    if (userRoleList.isEmpty) {
      // Add default subscriber role if no roles are configured
      userRoleList = [
        {"option": "Subscriber", "value": "subscriber"},
        {"option": "Agent", "value": "houzez_agent"},
        {"option": "Agency", "value": "houzez_agency"},
      ];
      _roleValue = "subscriber"; // Set default
    } else if (userRoleList.length == 1) {
      _roleValue = userRoleList[0];
    }
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      // Dropdown removed - using default role
  ...
  Widget dropDownRole(List list, String title, String hintText){
    return GenericStringDropDownWidget(
      ...
      items: list.map<DropdownMenuItem<String>>((item) {
        // Handle both Map and String items for backward compatibility
        String displayText;
        String itemValue;
        
        if (item is Map) {
          displayText = item["option"] ?? item["value"] ?? "Unknown";
          itemValue = item["value"] ?? item["option"] ?? "";
        } else {
