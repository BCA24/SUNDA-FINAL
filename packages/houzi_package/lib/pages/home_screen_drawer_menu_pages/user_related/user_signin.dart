...
class UserSignInState extends State<UserSignIn> with ValidationMixin {
  ...
  Future<void> onSignInPressed() async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      // Use PostgreSQL backend for login
      print('🔐 PostgreSQL Login - Validating credentials...');
      print('📧 Username: "$loginUsername"');
      print('🔐 Password: "${loginPassword.isEmpty ? "[EMPTY]" : "[PROVIDED]"}"');
      ...
      final result = await PostgresqlAuthService.login(
        email: loginUsername,
        ...
        setState(() {
          ...
          if (result['success'] == true) {
            ...
            Map<String, dynamic> userLoginData = {
              'user_id': result['user']['id'], // Keep as int, not string
              'user_email': result['user']['email'],
              'user_login': result['user']['email'],
              'user_display_name': result['user']['first_name'] ?? result['user']['email'],
              'user_first_name': result['user']['first_name'] ?? '',
              'user_last_name': result['user']['last_name'] ?? '',
              'user_role': result['user']['role'] ?? 'user',
            };
            ...
            oneSignalLoginFunc(result['user']['email']);
