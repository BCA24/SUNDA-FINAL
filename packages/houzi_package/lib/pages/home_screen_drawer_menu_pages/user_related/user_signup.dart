...
// Import PostgreSQL auth service
...
class UserSignUpState extends State<UserSignUp> with ValidationMixin {
  ...
  void initState() {
    ...
    } else if (userRoleList.isEmpty || _roleValue == null) {
      // Set default subscriber role if no roles configured or multiple roles
      _roleValue = "subscriber";
    }
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      // Dropdown removed - using default subscriber role
    ...
    return ButtonWidget(
      ...
      onPressed: () async {
        ...
        if (formKey.currentState!.validate()) {
          ...
          // Use PostgreSQL backend instead of WordPress
          print('🚀 Attempting registration with PostgreSQL backend...');
          print('👤 Username: $userName');
          print('📧 Email: $email');
          
          final result = await PostgresqlAuthService.register(
            username: userName,
            email: email,
            firstName: firstName.isEmpty ? userName : firstName,
            lastName: lastName,
            phone: phoneNumber,
            password: registerPass.text.isEmpty ? 'defaultpass123' : registerPass.text,
          );

          if (mounted) {
            setState(() {
              _showWaitingWidget = false;
              _isInternetConnected = true; // Assume connected if we got a response
            });
          }
          
          if (result['success'] == true) {
            print('✅ Registration successful!');
            _showToast(context, result['message'] ?? 'Registration successful! Please login.');
            ...
            _showToast(context, result['message'] ?? 'Registration failed. Please try again.');
          }
        }
      },
    );
