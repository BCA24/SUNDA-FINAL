...
class _QuickAddPropertyState extends State<QuickAddProperty> {
  ...
  void initState() {
    ...
    // CLEAR OLD QUEUED PROPERTIES - They don't have price data!
    print('🧹 CLEARING old queued properties from storage...');
    List<dynamic>? oldProperties = HiveStorageManager.readAddPropertiesDataMaps();
    if (oldProperties != null && oldProperties.isNotEmpty) {
      print('🧹 Found ${oldProperties.length} old properties in queue - CLEARING THEM!');
      HiveStorageManager.storeAddPropertiesDataMaps([]);
      print('✅ Queue cleared! Fresh start.');
    } else {
      print('✅ No old properties in queue.');
    }
    ...
    return Container(
      ...
      child: TextFormFieldWidget(
        ...
        validator: (String? value) {
          ...
          // IMMEDIATELY save price to map when validated
          if (value != null && value.isNotEmpty) {
            int parsedPrice = int.parse(value.replaceAll(RegExp(r'[^0-9]'), ''));
            setState(() {
              _price = value;
              addPropertyDataMap['sale_price'] = parsedPrice;
              addPropertyDataMap['fave_property_price'] = value;
              addPropertyDataMap[ADD_PROPERTY_PRICE] = value;
              addPropertyDataMap['prop_price'] = value;
              addPropertyDataMap['price'] = value;
            });
            print('💰 PRICE SET IN VALIDATOR: $_price (int: $parsedPrice)');
            print('💰 addPropertyDataMap now contains: sale_price=${addPropertyDataMap['sale_price']}');
          }
          return null;
        },
        onSaved: (value) {
          setState(() {
            _price = value ?? '';
            print('DEBUG: Prijs ingevuld in formulier: $_price');
          });
        },
        onChanged: (value) {
          // ALSO save immediately on change
          if (value != null && value.isNotEmpty) {
            try {
              int parsedPrice = int.parse(value.replaceAll(RegExp(r'[^0-9]'), ''));
              setState(() {
              ...
              print('💰 PRICE CHANGED: $value (int: $parsedPrice)');
            } catch (e) {
  ...
  startAddPropertyProcess(BuildContext context) {
    if (formKey.currentState!.validate()) {
      ...
      print('DEBUG: [startAddPropertyProcess] _price after save: $_price');
      if(mounted) {
        setState(() {
          addPropertyDataMap[ADD_PROPERTY_TITLE] = _title;
          addPropertyDataMap[ADD_PROPERTY_PRICE] = _price;
          print('DEBUG: [startAddPropertyProcess] _price value before sending: $_price');
          int parsedPrice = 0;
          if (_price.isNotEmpty) {
            try {
              parsedPrice = int.parse(_price.replaceAll(RegExp(r'[^0-9]'), ''));
              print('DEBUG: [startAddPropertyProcess] parsedPrice: $parsedPrice');
            } catch (e) {
              print('WARNING: Could not parse _price ($_price)');
            }
          } else {
            print('WARNING: _price is empty');
          }
          // sale_price moet altijd een int zijn, niet een string!
          addPropertyDataMap['sale_price'] = parsedPrice;
          print('DEBUG: [startAddPropertyProcess] addPropertyDataMap[sale_price] (int): ${addPropertyDataMap['sale_price']}');
          addPropertyDataMap['fave_property_price'] = _price;
          addPropertyDataMap['prop_price'] = _price;
          addPropertyDataMap['price'] = _price;
          print('DEBUG: [startAddPropertyProcess] addPropertyDataMap[sale_price]: ${addPropertyDataMap['sale_price']}');
          print('DEBUG: [startAddPropertyProcess] addPropertyDataMap[fave_property_price]: ${addPropertyDataMap['fave_property_price']}');
          print('DEBUG: [startAddPropertyProcess] addPropertyDataMap[prop_price]: ${addPropertyDataMap['prop_price']}');
          print('DEBUG: [startAddPropertyProcess] addPropertyDataMap[price]: ${addPropertyDataMap['price']}');
          ...
          addPropertyDataMap['fave_property_bedrooms'] = _bedrooms;
          addPropertyDataMap[ADD_PROPERTY_BATHROOMS] = _bathrooms;
          addPropertyDataMap['fave_property_bathrooms'] = _bathrooms;
          addPropertyDataMap['property_map_address'] = _address;
          print('DEBUG: [startAddPropertyProcess] addPropertyDataMap:');
          addPropertyDataMap.forEach((key, value) {
            print('  $key: $value');
          });
        });
      }
      ...
      if (isLoggedIn == true) {
        ...
        // Debug: print de hele map vlak voor upload
        print('DEBUG: FINAL addPropertyDataMap before uploadProperty:');
        addPropertyDataMap.forEach((key, value) {
          print('  $key: $value');
        });
        if (_price.isEmpty || addPropertyDataMap['sale_price'] == 0) {
          print('WARNING: _price is empty of sale_price is 0!');
        }
